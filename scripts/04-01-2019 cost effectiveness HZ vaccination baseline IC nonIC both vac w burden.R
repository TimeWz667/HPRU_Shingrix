...
results_df<-data.frame(total_QG_HZ_d=rep(NA,times=n.run),
...
results_df$total_net_cost<-total_costs_intervention-results_df$total_saved_costs_d
...
##########################
#### save output
write.csv(results_df, file=paste(Output,"09-01-2019_CEA_", vaccine, "_", IC_status, "_", "age", vaccination_age, ".csv", sep=""))


finished_run<-paste(Output,"09-01-2019_CEA_", vaccine, "_", IC_status, "_", "age", vaccination_age, ".csv", sep="")
print(finished_run)
...
# for (i in 1: length(l)){
#   p_HZ_alive[i]<-sum(l[[i]]$p_HZ_alive)*(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)
# }
# mean(p_HZ_alive)
# quantile(p_HZ_alive, prob=c(0.05,0.5, 0.95))
# p_HZ_alive<-paste(round(quantile(p_HZ_alive, prob=c(0.5)),1),
#                        " (",round(quantile(p_HZ_alive, prob=c(0.05)),1),"-", 
#                        round(quantile(p_HZ_alive, prob=c(0.95)),1),")", sep="")
# 
# cohort_size<-(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop) * vaccine_coverage
# 
# # N cases HZ post vac
...
#     (sum(l[[i]]$p_HZ_alive)*((Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)-cohort_size)) # is p_HZ_alive here correct?
#   
# }
# mean(HZ_post_vac)
# quantile(HZ_post_vac, prob=c(0.05,0.5, 0.95))
# HZ_post_vac<-paste(round(quantile(HZ_post_vac, prob=c(0.5)),1),
#                               " (",round(quantile(HZ_post_vac, prob=c(0.05)),1),"-", 
#                               round(quantile(HZ_post_vac, prob=c(0.95)),1),")", sep="")
# 
# 
# # N hospitalisations pre vac
# Hospitalisation<-rep(NA, length(l))
# for (i in 1: length(l)){
#   Hospitalisation[i]<-sum(l[[i]]$p_hospitalisation)*(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)
# }
# mean(Hospitalisation)
# quantile(Hospitalisation, prob=c(0.05,0.5, 0.95))
# Hospitalisation<-paste(round(quantile(Hospitalisation, prob=c(0.5)),1),
#                                   " (",round(quantile(Hospitalisation, prob=c(0.05)),1),"-", 
#                                   round(quantile(Hospitalisation, prob=c(0.95)),1),")", sep="")
# 
...
#     (sum(l[[i]]$p_hospitalisation)*((Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)-cohort_size))
#   
# }
# mean(Hospitalisation_post_vac)
# quantile(Hospitalisation_post_vac, prob=c(0.05,0.5, 0.95))
# Hospitalisation_post_vac<-paste(round(quantile(Hospitalisation_post_vac, prob=c(0.5)),1),
#                                            " (",round(quantile(Hospitalisation_post_vac, prob=c(0.05)),1),"-", 
#                                            round(quantile(Hospitalisation_post_vac, prob=c(0.95)),1),")", sep="")
# 
# #GP cost undiscounted pre vac
# Cost_GP<-rep(NA, length(l))
# for (i in 1: length(l)){
#   Cost_GP[i]<-sum(l[[i]]$Cost_GP)*(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)
# }
# mean(Cost_GP)
# quantile(Cost_GP, prob=c(0.05,0.5, 0.95))
# Cost_GP<-paste(round(quantile(Cost_GP, prob=c(0.5)),1),
#                           " (",round(quantile(Cost_GP, prob=c(0.05)),1),"-", 
#                           round(quantile(Cost_GP, prob=c(0.95)),1),")", sep="")
# 
# 
...
#     (sum(l[[i]]$Cost_GP)*((Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)-cohort_size))
#   
# }
# mean(Cost_GP_post_vac)
# quantile(Cost_GP_post_vac, prob=c(0.05,0.5, 0.95))
# Cost_GP_post_vac<-paste(round(quantile(Cost_GP_post_vac, prob=c(0.5)),1),
#                                    " (",round(quantile(Cost_GP_post_vac, prob=c(0.05)),1),"-", 
#                                    round(quantile(Cost_GP_post_vac, prob=c(0.95)),1),")", sep="")
# 
# #Hospital cost undiscounted pre vac
# Cost_hospitalisation<-rep(NA, length(l))
# for (i in 1: length(l)){
#   Cost_hospitalisation[i]<-sum(l[[i]]$Cost_hospitalisation)*(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)
# }
# mean(Cost_hospitalisation)
# quantile(Cost_hospitalisation, prob=c(0.05,0.5, 0.95))
# Cost_hospitalisation<-paste(round(quantile(Cost_hospitalisation, prob=c(0.5)),1),
#                                        " (",round(quantile(Cost_hospitalisation, prob=c(0.05)),1),"-", 
#                                        round(quantile(Cost_hospitalisation, prob=c(0.95)),1),")", sep="")
# 
...
#     (sum(l[[i]]$Cost_hospitalisation)*((Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)-cohort_size))
#   
# }
# mean(Cost_hospitalisation_post_vac)
# quantile(Cost_hospitalisation_post_vac, prob=c(0.05,0.5, 0.95))
# Cost_hospitalisation_post_vac<-paste(round(quantile(Cost_hospitalisation_post_vac, prob=c(0.5)),1),
#                                                 " (",round(quantile(Cost_hospitalisation_post_vac, prob=c(0.05)),1),"-", 
#                                                 round(quantile(Cost_hospitalisation_post_vac, prob=c(0.95)),1),")", sep="")
# 
# 
# #QL pre vac undiscounted
# QL_HZ<-rep(NA, length(l))
# for (i in 1: length(l)){
#   QL_HZ[i]<-sum(l[[i]]$QL_HZ)*(Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)
# }
# mean(QL_HZ)
# quantile(QL_HZ, prob=c(0.05,0.5, 0.95))
# QL_HZ<-paste(round(quantile(QL_HZ, prob=c(0.5)),1),
#                         " (",round(quantile(QL_HZ, prob=c(0.05)),1),"-", 
#                         round(quantile(QL_HZ, prob=c(0.95)),1),")", sep="")
# 
# #QL post vac undiscounted
# QL_HZ_post_vac<-rep(NA, length(l))
# for (i in 1: length(l)){
#   QL_HZ_post_vac[i]<-(sum(l[[i]]$QL_HZ_post_vac)*cohort_size)+
#     (sum(l[[i]]$QL_HZ)*((Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop)-cohort_size))
#   
# }
# mean(QL_HZ_post_vac)
# quantile(QL_HZ_post_vac, prob=c(0.05,0.5, 0.95))
# QL_HZ_post_vac<-paste(round(quantile(QL_HZ_post_vac, prob=c(0.5)),1),
#                                  " (",round(quantile(QL_HZ_post_vac, prob=c(0.05)),1),"-", 
#                                  round(quantile(QL_HZ_post_vac, prob=c(0.95)),1),")", sep="")
# 
# 
# 
# BoD<-data.frame(Scenario=c("No vaccination", "vaccination"),
#                 N_HZ_cases=c(p_HZ,HZ_post_vac),
...
#                 Cost_intervention=c(0,total_costs_intervention))
# 
# # write.csv(BoD, file=paste(Output, "09-01-2019 Burden of disease table.csv", sep=""))
