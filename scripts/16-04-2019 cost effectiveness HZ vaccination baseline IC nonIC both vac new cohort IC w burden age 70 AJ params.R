...
k<-70# for (k in 50:90){
  ...
  # expand for ages we don't have
  younger<-data.frame(age=50:59,
                      Incidence_HZ=rep(Incidence_HZ$Incidence_HZ[1], length(50:59)),
                      Incidence_HZ_nIC=rep(Incidence_HZ$Incidence_HZ_nIC[1], length(50:59)))
  older<-data.frame(age=100,
                      Incidence_HZ=Incidence_HZ$Incidence_HZ[nrow(Incidence_HZ)],
                      Incidence_HZ_nIC=Incidence_HZ$Incidence_HZ_nIC[nrow(Incidence_HZ)])
  Incidence_HZ_2<-rbind(younger,Incidence_HZ, older)
  # put into list
  df<-data.frame(age=rep(50:100,n.run),
                 Incidence_HZ_nIC=rep(1:n.run, each=length(50:100)))
  l<-split(df, df$Incidence_HZ_nIC)
  
  for(i in 1:n.run){
        l[[i]]<-data.frame(age=50:100,
                           p_HZ=Incidence_HZ_2$Incidence_HZ_nIC)
      }
  ## Data Nick
  ...
  # in AJs model no difference, therefore
  for(i in 1:n.run)
  {l[[i]]$p_HZ_GP_only<-l[[i]]$p_HZ}
  ...
  younger<-data.frame(age=50:60,
                      QL_HZ=rep(QL_HZ$QL_HZ[1],length(50:60)))
  older<-data.frame(age=100,
                      QL_HZ=QL_HZ$QL_HZ[nrow(QL_HZ)])
  QL_HZ<-rbind(younger,QL_HZ, older)
  names(QL_HZ)<-c("age", "QL_HZ_d")
  l<-lapply(l, merge, y=QL_HZ, by="age")
  ...
  # l[[i]]$QL_HZ<-l[[i]]$QL_y1+l[[i]]$QL_y2
  # l[[i]]$QL_HZ_d<-l[[i]]$QL_y1+l[[i]]$QL_y2_d
  # 
  # }
  ...
  # add additional QALY loss directly to list (to avoid rounding errors)
  additional_QL_HZ_6m<-read.csv(file=paste(Data,"23-07-2019 AJs additional QL over 6m zvl.csv", sep=""))
  l<-lapply(l, merge, y=additional_QL_HZ_6m, by="age")
  ...
  # l[[i]]$p_deaths_HZ<-1-exp(-l[[i]]$Incidence_HZ_mortality) #conversion instantaneous rate to probability
  # }
  ...
  l<-lapply(l, merge, y=Hospitalisation_rate_HZ, by="age")
  ...
  # l[[i]]$p_hospitalisation<-1-exp(-l[[i]]$Hospitalisation_rate_HZ) #conversion instantaneous rate to probability
  # }
  ...
  names(Hospitalisation_costs_HZ)<-c("age", "Hospitalisation_costs_pp_HZ_inf")
  l<-lapply(l, merge, y=Hospitalisation_costs_HZ, by="age")
  ...
  l<-lapply(l, merge, y=Cost_GP_per_HZ, by="age")
  ...
  if(IC_status=="non IC"){
    target_population<-Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop
  } else if (IC_status=="IC"){
    target_population<-Pop_2015[Pop_2015$Age%in%vaccination_age,]$Pop_IC
  } else {print("Error")}
  ...
  # l[[x]]$p_deaths_HZ<-l[[x]]$p_HZ*l[[x]]$Proportion_deaths
  ...
  if (AJ=="Y"){l[[x]]$p_hospitalisation<-l[[x]]$p_HZ_alive*l[[x]]$Hospitalisation_rate_HZ}
  ...
  l[[x]]$QL_HZ_pre_vac<-l[[x]]$p_HZ_alive*l[[x]]$QL_HZ_d
  
  # discount QALYs
  l[[x]]$QL_HZ_pre_vac_d<-l[[x]]$QL_HZ_pre_vac/((1+discount_rate_effects)^(l[[x]]$age-vaccination_age))
 ...
 if(AJ=="N"){
  ...
  l[[x]]$additional_QL_o3m_d<-l[[x]]$QL_o3m_post_vac_no_VE_d-l[[x]]$QL_o3m_post_vac_with_VE_d
    ...
    # l[[x]]$additional_QL_o3m_d<-(l[[x]]$p_HZ_post_vac*l[[x]]$QL_6m_pre_vac)-(l[[x]]$p_HZ_post_vac*l[[x]]$QL_6m_post_vac*0.646914809)
    l[[x]]$additional_QL_o3m_d<-l[[x]]$additional.QL.6m
    }
  ...
  l[[x]]$QL_death_pre_vac<-l[[x]]$p_deaths_HZ*l[[x]]$QL_death
  
  # Discounted
  l[[x]]$QL_death_pre_vac_d<-l[[x]]$QL_death_pre_vac/((1+discount_rate_effects)^(l[[x]]$age-vaccination_age))
  ...
  l<-lapply(1:n.run, QL_death_calc)
    ...
    for(i in 1:n.run){
      l[[i]]$QL_death_pre_vac_d<-0
      l[[i]]$QL_death_post_vac_d<-0
    }
  }
  ...
  results_df[i,"total_QL_no_vac_d"]<-(sum(l[[i]]$QL_HZ_pre_vac_d+l[[i]]$QL_death_pre_vac_d))*cohort_size
  ...
  results_df[i,"total_QG_HZ_d"]<-sum(l[[i]]$QL_HZ_pre_vac_d-l[[i]]$QL_HZ_post_vac_d)
  
  # QALYs gained for death of HZ for each age summed (or zero)
  results_df[i,"total_QG_death_HZ_d"]<-sum(l[[i]]$QL_death_pre_vac_d-l[[i]]$QL_death_post_vac_d)
  ...
  # p only PHN and only HZ (no PHN)
  l[[x]]$p_PHN_post_vac<-(l[[x]]$p_HZ_GP_only_alive*(1-l[[x]]$VE))*(1-l[[x]]$VE_PHN_w_HZ)*l[[x]]$p_PHN #different to p_PHN_AJ
  l[[x]]$p_non_PHN_HZ_post_vac<-(l[[x]]$p_HZ_GP_only_alive*(1-l[[x]]$VE))*(1-((1-l[[x]]$VE_PHN_w_HZ)*l[[x]]$p_PHN))
  ...
  # write.csv(BoD, file=paste(Output,"02-07-2019_Burden_of_disease_table_", vaccine, "_", IC_status, "_", "age", vaccination_age, ".csv", sep=""))
