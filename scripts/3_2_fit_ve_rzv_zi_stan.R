...
post <- sampling(model, data = ds, iter = 10000)

post

save(post, file = here::here("outputs", "temp", "pars_ve_zlg.rdata"))
...
sel %>% 
  filter(Key <= 3000) %>% 
  write_csv(here::here("pars", "pars_ve_zlgamma.csv"))


sel %>% 
  filter(Key <= 1000) %>% 
  full_join(crossing(Yr = 0:100, Key = 1:1000)) %>% 
  mutate(
    VE = p0 * (1 - pgamma(Yr, alpha, beta))
  )  %>% 
  select(Key, Yr, VE) %>% 
  write_csv(here::here("pars", "sims_ve_zlgamma.csv"))
...
ggsave(g_gof, filename = here::here("results", "figs", "g_pars_ve_zl_gamma.png"), width = 7, height = 5.5)
