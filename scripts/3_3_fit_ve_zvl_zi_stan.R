...
sims <- sel %>% 
  ...
  select(Key, Yr, VE) %>% 
  write_csv(here::here("pars", "sims_ve_zvl_zlgamma.csv"))
...
g_gof <- sims %>% 
  ...
  geom_pointrange(data = dat_ve %>% filter(Follow_up == 1), aes(x = Yr, y = M, ymin = L, ymax = U)) +
...
ggsave(g_gof, filename = here::here("outputs", "figs", "g_pars_ve_zvl_zlgamma.png"), width = 7, height = 5.5)
